 ...
 * do a fallback action if provided directly on a provided Table_ref.
 * Skip temporary tables during processing.
 */
using action_fn = std::function<void(const dd::Table *)>;
using fallback_action_fn = std::function<void(Table_ref *, const char *)>;
      ...
      */
...

      {
        ...
        if (table_obj != nullptr) {
          action(table_obj);
          if (table_se) {
            *table_se = table_obj->engine().c_str();
          }
        } else if (fallback_action) {
          fallback_action(table, table_se ? *table_se : nullptr);
        }
...
bool wsrep_append_child_tables(THD *thd, Table_ref *table,
                               wsrep::key_array *keys, const char **table_se) {
  ...
  auto populate_child_tables_fallback = [thd](const Table_ref *table_ref,
    ...
    if (thd->dd_client()->fetch_fk_children_uncached(
            ...
            se ? se : "InnoDB", true, &children_dbs, &children_names)) {
      return;
    }
  ...
  if (wsrep_do_action_for_table(thd, table, populate_child_tables,
                                populate_child_tables_fallback, table_se))
    return true;
