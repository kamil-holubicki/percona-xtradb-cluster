      ...
      */
      if (!thd->is_current_stmt_binlog_format_row() ||
        ...
        bool should_start_toi = !prepared_stmt_execution
          || (prepared_stmt_execution && !wsrep_thd_is_in_to_isolation(thd, false));
        /* Note we are explicitly opening the macro as we need to perform
        cleanup action on TOI failure. */ // KH: here TOI begin
        if (WSREP(thd) && should_start_toi && wsrep_to_isolation_begin(thd, create_table->db,
                                                   create_table->table_name,
