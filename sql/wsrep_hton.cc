...
static int wsrep_prepare(handlerton *hton, THD *thd, bool all)
{
  ...
  assert(thd->ha_data[wsrep_hton->slot].ha_info[all].is_trx_read_write());
...
wsrep_run_wsrep_commit(THD *thd, handlerton *hton, bool all)
{
  ...
  else if (!rcode)
  {
    if (WSREP_OK == rcode) {
      ...
      rcode = wsrep->replicate_pre_commit(wsrep,
    ...
    assert(0); // failure like this can not normally happen
    DBUG_RETURN(WSREP_TRX_ERROR);
  }
  ...
  case 0:
    ...
    */
    ...
    if (thd->get_transaction()->xid_state()->get_xid()->is_null() ||
