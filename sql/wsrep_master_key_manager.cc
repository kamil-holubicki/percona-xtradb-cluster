...
MasterKeyManager::MasterKeyManager(std::function<void(const std::string&)> logger)
: logger_(logger)
{}
...
bool MasterKeyManager::GenerateKey(const std::string& keyId) {
  if (keyring_generator_service_->generate(
          keyId.c_str(), nullptr,
          key_type, KEY_LEN) == true) {
    logger_("MasterKey generation FAILED. Check if Keyring component "
                "is installed.");
    return true;
  }
  return false;
